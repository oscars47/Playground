...
def build_eq(op_temp, inp):
    # print(op_temp)
    eqn = ""

    op_int = 0
    inp_int = 0 # initialize index for input
    while (op_int < len(op_temp)) and (inp_int < len(inp)):
        temp_split = ''
        current_op = op_temp[op_int]
        # next_op = op_temp[op_int+1]
        if op_int==0 and current_op !=0: # if it's 0, figure out how many more 0s so inp values to keep appending
            eqn+=op_dict[str(current_op)]
            
        if current_op != -2: # if not open (
            temp_split+=str(inp[inp_int])
            # inp_int+=1
            # op_int+=1
        ...
        for op_next_int in range(op_int+1, len(op_temp), 1):
            ...
            if (op_next_int == len(op_temp)-1):
                if next_op==0:
                    ...
                    eqn+=temp_split
                    eqn+=op_dict[str(op_temp[op_next_int])]
                    inp_int+=1
                    eqn+=str(inp[inp_int])
                ...
                # print('in loop', inp_int)
                # print('temp_split', temp_split)
                # print('eq before', eqn)
                # print('eq last', eqn[-1])
                # check if anything in temp_split
                if eqn[-1] in ['-', '+', '*', '/', '**']:
                    eqn+=temp_split
                    eqn+=op_dict[str(op_temp[op_next_int])]
                else:
                    if op_temp[op_next_int] == -3: # if )
                        eqn+=op_dict[str(op_temp[op_next_int])]
                        eqn+=op_dict[str(op_temp[op_next_int+1])]
                        eqn+=str(inp[inp_int])
                    else:
                        eqn+=op_dict[str(op_temp[op_next_int])]
                        eqn+=str(inp[inp_int])
...
def update(p_ls, inp): # takes in current p list and input numbers
    ...
    def compute_int_val(op_temp, inp):
        eqn = build_eq(op_temp, inp)
        eqn = "x="+eqn
        #print(eqn)
    ...
    def compute_all(op_temp):# go through and compute
            ...
            if len(op_temp) > 0:
                if is_not_leading_0(op_temp, inp): # if splitting doesn't result in leading 0s
                    ...
                    int_val = compute_int_val(op_temp, inp) # compute the integer result
...
def print_results():
    ...
    results = list(zip(int_val_all, inp_all, op_perm_all))
